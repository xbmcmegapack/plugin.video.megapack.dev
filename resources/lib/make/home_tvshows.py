...
class Home_tvshows():
    ...
    def make(self):
        """ Generate Home_tvshows submenu files.
        ...
        for menu in TOPICS:
            ...
            f = codecs.open(xbmcaddon.Addon().getAddonInfo('path') + \
                    PLUGIN_MENUS_FOLDER + 'home_tvshows_' + name.lower() + \
...
\n\
    def open(self, plugin, menu):\n\
        menu.add_xplugins(plugin.get_xplugins(dictionaries=["TVShows"],\n\
            topics=["' + name.capitalize() + '"],\n\
            languages=["My"], countries=["My"]))\
'
            )
            f.close()
        """Generate Home_tvshows menu file."""
        file = open(xbmcaddon.Addon().getAddonInfo('path') + \
                PLUGIN_MENUS_FOLDER + 'home_tvshows.py', 'w')
        file.write(PYTHON_HEADER + LICENCE)
        for menu in TOPICS:
            file.write(\
"\
from home_tvshows_" + _str.normalize(menu).lower() + " import *\n\
"
            )
        file.write("\n\n")
        file.write(\
